async function e(){try{let e=await fetch(`./common-cmds.json`);if(!e.ok)throw Error(`Error JSON common-cmds`);let t=await e.json(),n=document.getElementById(`common-cmds-container`);if(!n)return;n.innerHTML=t.map((e,t)=>{let n=t%2==0?``:`bg-even`,r=e.synthax?`<p><span class="font-semibold mt-3 synth">Synthax: </span>${e.synthax}</p>`:``,i=``;return e.aliases&&e.aliases.forEach((t,n)=>{i+=`<span>${t}</span>`,n!=e.aliases.length-1&&(i+=` - `)}),`
        <div class="flex w-full common-cmds ${n}">
          <div
        ...
        `}).join(``)}catch(e){console.error(e)}}export{e as initCommonCmds};